...
export interface RepositoryInterviewOperatorCandidatePersistencePortV1 {
  loadArtifactContext(
...
export interface RepositoryInterviewOperatorPersistencePortV1 {
  ...
  forCandidate(
    signal: AbortSignal,
  ): RepositoryInterviewOperatorCandidatePersistencePortV1;
}
...
export function createRepositoryInterviewPersistenceAdapterV1(
  ...
  return Object.freeze({
    ...
    forCandidate(signal: AbortSignal) {
      ...
      const candidateControl: OperationControl = Object.freeze({
        ...control,
        signal,
      });
      const record: RepositoryInterviewRecordPortV1 = Object.freeze({
        async findReusable(lookup: RepositoryInterviewReuseLookupV1) {
          try {
            ...
            const result = await findReusableRepositoryInterview(
              client,
              lookup,
              candidateControl,
            );
        ...
        async loadArtifactContext(
          ...
          try {
            ...
            if (
              ...
              parsedSet.value.identityDigest !==
...
function rejectAborted(signal: AbortSignal): void {
  if (signal.aborted) throw new RepositoryInterviewOperatorPersistenceError();
}
