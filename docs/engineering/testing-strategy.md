...
| Phase 8 retrieval foundation | `pnpm taxonomy:validate` source/schema/semantic/order/digest/drift closure; exact taxonomy/alias graph and abuse tests; query DTO bounds and hostile-object preflight; ASCII canonicalization, mixed exclusion and blocking-outcome closure, 50-unresolved capacity, exact candidate-ID references, modality/source preservation, standalone semantic-forgery rejection, fingerprint non-inference, digest and exchange closure; `pnpm profiles:validate` exact 27-field/state/source/value semantics, 150-candidate closure, byte-identical generation, symlink/size/path denial, 4,050-cell arithmetic, prose non-authority, schema-digest compatibility, and conservative single-candidate constraint evaluation; later normalization/clarification gold, independent relevance judgments, baselines, and content-free reports |
| Future retrieval and ranking | Production candidate generation, retrieval, ranking, deterministic tie behavior, performance, and codebase-conditioned quality evidence after separate architecture approval                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
...
matrix, and selected generated results receive independent human audit.
...
hard-filter violations, no-viable-candidate accuracy, clarification accuracy,
alias-expansion correctness, and prohibited-constraint preservation. A zero
denominator is null or not applicable and is excluded from macro means, never
silently 1.0. Offline deterministic baselines produce a drift-checked,
