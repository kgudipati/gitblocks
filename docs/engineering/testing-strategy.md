...
graph. The repository's production-owned code includes the pure domain and
contract kernel, the concrete PostgreSQL persistence adapter, the operator-run
public ingestion/artifact adapter, the persistence-independent interview
package, and the pure retrieval package. Their lifecycle classifications
determine which checks are focused during development; they do not remove
coverage from the authoritative final verification graph. Each future product
slice must extend this strategy for the services and boundaries it actually
introduces before that code lands.
...
- For a product slice, the first validation target is the intended
  user-visible outcome at the nearest supported boundary. Component tests,
  evaluation authority, or infrastructure proof cannot substitute for an
  exercisable journey.
- During implementation, run the smallest focused checks that cover changed
  behavior and affected boundaries. Run full repository verification as the
  final regression and review gate where appropriate. This changes neither the
  authoritative final graph nor any existing safety threshold.
- Add infrastructure test machinery only for a concrete current blocker with
  observed evidence. Missing information or hypothetical scale is not test
...
evaluation. Real PostgreSQL 18 tests apply all five migrations and preserve the
...
The PostgreSQL 18.4 suite applies migrations through `0005` in the existing test
harness, then preserves the 6/30/150 selection materialization proof from one complete
...
Recovery R3 adds a current migration-`0005` serving integration over the
accepted 150-candidate catalog. It proves four checked tables, deferred complete
snapshot closure, exact publication and replay, immutable conflict rejection,
historical readability, a `gitblocks_serving` login that can SELECT but cannot
write or perform DDL, and fail-closed reconstruction after profile or metadata
corruption. The same no-network suite parses the committed profile/metadata
authorities, builds `createCandidateRetrievalEngineV1(...)` from a read-only
PostgreSQL load, and compares the complete representative result with the
committed-authority path. Bootstrap command tests expose no provider, model, or
network capability.
...
| MCP surface                     | R5 proves official `2026-07-28` client negotiation over real loopback Streamable HTTP, exact single-tool listing, canonical input-schema projection, one application call per tool call, preserved clarification/unsupported/retrieved outcomes and shortlist digest, bounded redacted failures, immutable application reuse after PostgreSQL access is revoked, readiness-before-listen, listener-first shutdown, and fixed loopback binding; authorization, tenant isolation, cancellation, pagination, and other features are tested only when introduced                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
...
| Hosted retrieval and target fit | Existing pure retrieval keeps its deterministic channel, lane, hard-filter, provenance, performance, and determinism coverage; R3 adds fresh PostgreSQL bootstrap, coherent read-only serving reconstruction, least-privilege denial, corruption closure, and committed-versus-loaded equivalence; R4 adds structured request-boundary parsing/normalization, preserved clarification/unsupported outcomes, hard exclusion, separate eligible/evidence-needed lanes, startup load-once, post-start database isolation, failure closure, deterministic replay, and client shutdown; later fit work adds bounded model-output validation and critical end-to-end journey coverage                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
