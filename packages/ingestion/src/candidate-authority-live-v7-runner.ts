...
export interface CandidateAuthoritySuccessorPreflightResultV7 extends CandidateAuthoritySuccessorPreflightResult {
  readonly providerRoutes: CandidateAuthorityProviderRoutes;
}

export interface CandidateAuthoritySuccessorEffectsV7 extends Omit<
  CandidateAuthoritySuccessorEffects,
  'collect' | 'preflight'
> {
  readonly preflight: (
    acceptedHead: string,
  ) => Promise<CandidateAuthoritySuccessorPreflightResultV7>;
  readonly collect: (
    input: Omit<
...
export interface CandidateAuthoritySuccessorFailureDiagnosticV7 {
  readonly status: 'failed';
  readonly operatorVersion: typeof CANDIDATE_AUTHORITY_LIVE_OPERATOR_V7_VERSION;
  readonly authorizationVersion: typeof CANDIDATE_AUTHORITY_LIVE_AUTHORIZATION_V7_VERSION;
  readonly authorizationDigest: typeof CANDIDATE_AUTHORITY_LIVE_AUTHORIZATION_V7_DIGEST;
...
export class CandidateAuthoritySuccessorExecutionFailureV7 extends Error {
  public readonly diagnostic: CandidateAuthoritySuccessorFailureDiagnosticV7;
  public constructor(
    diagnostic: CandidateAuthoritySuccessorFailureDiagnosticV7,
  ) {
    super('Candidate authority canonical-routing successor execution failed.');
    this.name = 'CandidateAuthoritySuccessorExecutionFailureV7';
    this.diagnostic = diagnostic;
    Object.defineProperty(this, 'stack', { value: undefined });
  }
}

export async function preflightCandidateAuthoritySuccessorV7(
  effects: Pick<CandidateAuthoritySuccessorEffectsV7, 'preflight'>,
  acceptedHead: string,
): Promise<CandidateAuthoritySuccessorPreflightResultV7> {
...
export async function executeCandidateAuthoritySuccessorV7(
  effects: CandidateAuthoritySuccessorEffectsV7,
  acceptedHead: string,
  signal?: AbortSignal,
): Promise<CandidateAuthoritySuccessorSourceAuthority> {
  const preflight = await preflightCandidateAuthoritySuccessorV7(
    effects,
    acceptedHead,
  );
  ...
  try {
    ...
    throw new CandidateAuthoritySuccessorExecutionFailureV7(
      failureDiagnostic({
        preflight,
        cutoff,
        stage,
...
export function renderCandidateAuthoritySuccessorFailureV7(
  error: unknown,
): string {
  if (error instanceof CandidateAuthoritySuccessorExecutionFailureV7)
...
function failureDiagnostic(input: {
  readonly preflight: CandidateAuthoritySuccessorPreflightResultV7;
...
}): CandidateAuthoritySuccessorFailureDiagnosticV7 {
  const fatal =
