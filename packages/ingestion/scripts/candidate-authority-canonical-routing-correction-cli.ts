...
async function validateCorrection() {
  ...
  const fixed = parseCandidateAuthoritySuccessorFixedAuthorities({
    ...
    providerContractV5: text(CANDIDATE_AUTHORITY_PROVIDER_CONTRACT_V5_PATH),
    ...
    sourcePolicyV10: text(CANDIDATE_AUTHORITY_SOURCE_POLICY_V10_PATH),
    replayV7: text(CANDIDATE_AUTHORITY_REPLAY_V7_PATH),
    authorizationV8: text(CANDIDATE_AUTHORITY_LIVE_AUTHORIZATION_V8_PATH),
  });
  validateCandidateAuthorityCanonicalRoutingAuthorities({
